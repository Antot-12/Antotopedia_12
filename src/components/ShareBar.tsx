...
function NeonIconWrapper({ children }: { children: React.ReactNode }) {
    return (
        <div className="h-16 w-16 grid place-items-center rounded-xl border border-white/15 hover:border-accent hover:text-accent transition shadow-[0_0_0_0_rgba(46,231,216,0)] hover:shadow-[0_0_28px_0_rgba(46,231,216,.35)]">
            {children}
        </div>
    );
}
    ...
    return (
        <div className="card p-4 grid gap-4">
            ...
            <div className="flex items-center justify-center gap-2">
                <button className="btn btn-primary h-9 px-3" onClick={copy}>{copied ? "Copied" : "Copy link"}</button>
                <a className="btn btn-soft h-9 px-3" href={`mailto:?subject=${encodeURIComponent(title)}&body=${encodeURIComponent(url)}`}>Email</a>
            </div>
        </div>
    );
}
